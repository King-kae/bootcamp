...
export default function BootcampPage() {
  return (
    ...
    >
      ...
      {/* HERO */}
      <section id="top" className="py-20 bg-gradient-to-b from-slate-50 to-white border-b">
        <div className="mx-auto max-w-6xl px-6 text-center">
          ...
          <p className="text-slate-600 text-sm">Day 3 Venue: {CONFIG.location}</p>
          <p className="mt-4 text-lg text-slate-600 max-w-3xl mx-auto">
            Turn your ideas into live, automated trading systems. Over three days, gain hands‑on experience in strategy design, backtesting, risk management, and deployment—guided by industry experts and mentors.
          </p>
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button 
               className="h-12 rounded-xl px-6" aria-label="Download program outline">
              Download Outline
            </button>
          </div>
          <div className="mt-8 flex items-center justify-center gap-6 text-sm text-slate-600" role="list">
            <span className="inline-flex items-center gap-2" role="listitem">
              <MonitorSmartphone className="size-4" aria-hidden /> Day 1–2: Virtual
            </span>
            <span className="inline-flex items-center gap-2" role="listitem">
              <MapPin className="size-4" aria-hidden /> Day 3: Physical (Premium)
            </span>
          </div>
        </div>
      </section>
      ...
      <section id="overview" className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          >
            <Feature icon={<Cpu className="size-6" aria-hidden />} title="Systematic Strategy Design" desc="Learn frameworks for building robust, rules‑based trading systems." />
            <Feature icon={<LineChart className="size-6" aria-hidden />} title="Quant Tools & Pine Script" desc="Build and test strategies confidently on TradingView." />
            <Feature icon={<Clock className="size-6" aria-hidden />} title="Backtesting" desc="Validate with historical & live data to assess reliability and robustness." />
            <Feature icon={<ShieldCheck className="size-6" aria-hidden />} title="Risk Management" desc="Embed risk controls and guardrails from day one." />
            <Feature icon={<CalendarDays className="size-6" aria-hidden />} title="Performance" desc="Measure edge with clear metrics, attribution, and optimization loops." />
            <Feature icon={<Rocket className="size-6" aria-hidden />} title="Cloud Deployment" desc="Deploy 24/7 bots with monitoring and failover on cloud infrastructure." />
          </motion.div>
        </div>
      </section>
      ...
      <section id="benefits" className="py-20 bg-slate-50">
        <div className="mx-auto max-w-6xl px-6">
          <h2 className="text-3xl font-bold text-center">Why Join the Bootcamp?</h2>
          <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <Benefit icon={<GraduationCap className="size-6" aria-hidden />} title="Expert Mentorship" desc="Learn directly from quants and algo developers." />
            <Benefit icon={<Cpu className="size-6" aria-hidden />} title="Hands‑On Projects" desc="Work on real trading automation assignments." />
            <Benefit icon={<Users className="size-6" aria-hidden />} title="Community Access" desc="Join a network of aspiring algo traders and builders." />
            <Benefit icon={<LineChart className="size-6" aria-hidden />} title="Career‑Ready Portfolio" desc="Graduate with deployable strategies and proof of skill." />
          </div>
        </div>
      </section>
      ...
      <section id="timeline" className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <p className="mt-2 text-center text-slate-600">
            Three focused days—virtual foundations, design & testing, then in‑person deployment.
          </p>
          ...
          <div className="mt-10 grid lg:grid-cols-3 gap-6">
            <DayCard
              ...
              highlights={["Evolution of Trading Systems", "Career Pathways", "Core Concepts", "Basic Strategies"]}
            />
            <DayCard
              day="Day 2"
              mode="Virtual"
            ...
            <DayCard
              ...
              highlights={["Panel Discussion", "Advanced Workshop", "Deploy Live Strategies", "Mentorship & Networking"]}
            />
          </div>
        </div>
      </section>
          ...
          <div className="mt-10 grid sm:grid-cols-2 gap-6">
            <Card className="rounded-2xl">
              ...
              <CardContent className="text-slate-600 text-sm space-y-2">
                <p>
                  <span className="font-medium text-slate-800">Next Cohort:</span> <em>{CONFIG.cohortDates}</em>
                </p>
                <p>
                  <span className="font-medium text-slate-800">Format:</span> 2 Days Virtual + 1 Day Physical (Premium)
                </p>
                <p>
                  <span className="font-medium text-slate-800">Schedule:</span> Evenings & Weekend‑friendly
                </p>
              </CardContent>
            </Card>
      ...
      <section id="cta" className="py-20">
        <div className="mx-auto max-w-3xl px-6 text-center">
          <h3 className="text-3xl font-bold">Seats are limited—secure yours today.</h3>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-4 text-sm text-slate-600">
            <span className="inline-flex items-center gap-2">
              <CheckCircle2 className="size-4" aria-hidden /> <span>Mentorship Included</span>
            </span>
            <span className="inline-flex items-center gap-2">
              <CheckCircle2 className="size-4" aria-hidden /> <span>Hands‑On Labs</span>
            </span>
            <span className="inline-flex items-center gap-2">
              <CheckCircle2 className="size-4" aria-hidden /> <span>Certification</span>
            </span>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t py-10">
        <div className="mx-auto max-w-7xl px-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-8 text-sm">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              Systematic trading education & automation—built for real‑world deployment.
            </p>
          </div>
          <div>
        ...
        <div className="mt-8 text-center text-xs text-slate-500">
          © {new Date().getFullYear()} Leading Alpha Technologies LTD. All rights reserved.
        </div>
      </footer>
    </div>
  );
...
function HeaderNav() {
  return (
    <header
      className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b"
      aria-label="Primary"
    >
      <div className="mx-auto max-w-7xl px-6 flex h-16 items-center justify-between">
        <a href="#top" className="font-extrabold tracking-tight text-xl" aria-label="Leading Alpha Home">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className={`${THEME.btnPrimary} rounded-xl px-4`} aria-label="Get started">
            Get Started
          </button>
        </div>
      </div>
...
function LeadForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (paystack && CONFIG.paystackPublicKey) {
        ...
        alert("Thanks! We'll contact you shortly to finalize your reservation.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="mt-10 rounded-3xl border p-6 text-center">
      <h3 className="text-xl font-semibold">Reserve your seat</h3>
      <p className="mt-1 text-slate-600 text-sm">
        We’ll reach out within one business day with next steps.
      </p>
      <form className="mt-4 grid sm:grid-cols-4 gap-3" onSubmit={handleSubmit} noValidate>
        <input
        ...
        <button className={`${THEME.btnPrimary} h-12 rounded-xl`} type="submit" disabled={loading} aria-live="polite">
    ...
    <Card className="rounded-3xl overflow-hidden">
      <CardHeader className="bg-slate-50/60">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs rounded-full border px-3 py-1 bg-white">{mode}</span>
        </div>
      </CardHeader>
      <CardContent className="pt-6 text-slate-700 text-sm">
        <p className="mb-3">
          <span className="font-medium text-slate-900">Objective:</span> {objective}
        </p>
